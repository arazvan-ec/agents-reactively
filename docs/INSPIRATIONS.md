...
Este documento registra los artículos, ideas y textos que han influido en el desarrollo de este plugin y sus funcionalidades.

---

## Índice

1. [Compound Engineering - Ryan Carson](#1-compound-engineering---ryan-carson)

---

## 1. Compound Engineering - Ryan Carson

**Fecha de implementación**: 2026-01-29
**Autor original**: Ryan Carson (@ryancarson)
**Fuente**: Twitter/X - 28 enero 2026
**Componentes implementados**: Scripts de automatización, launchd templates, sistema de aprendizaje continuo

### Resumen

Ryan Carson describe un sistema donde el agente de IA:
- Trabaja después de cerrar el laptop
- Revisa el trabajo del día y extrae lecciones
- Actualiza sus propias instrucciones
- Implementa la siguiente prioridad del backlog
- Todo mientras el usuario duerme
...
|---------|-----------|-----------|
| `scripts/daily-compound-review.sh` | Revisión diaria de aprendizajes | Step 1 del artículo |
| `scripts/compound/auto-compound.sh` | Pipeline completo automatizado | Step 2 del artículo |
| `scripts/compound/analyze-report.sh` | Extracción de prioridad #1 | Mencionado en auto-compound |
| `scripts/compound/loop.sh` | Loop de ejecución iterativo | Mencionado como loop.sh |
| `launchd/*.plist` | Templates de automatización macOS | Step 3 y 4 del artículo |

#### Adaptaciones Realizadas

1. **Cambio de Amp a Claude Code**: Reemplazado `amp execute` por `claude -p --dangerously-skip-permissions`
2. **AGENTS.md → CLAUDE.md**: Adaptado para convención de Claude Code
3. **Enfoque en PHP**: Añadido contexto específico para desarrollo PHP
4. **Comandos como Skills**: Los comandos `/compound`, `/prd`, `/tasks` implementados como archivos en `.claude/commands/`
5. **Reintentos con backoff exponencial**: Añadido a operaciones de git para mayor robustez

#### Conceptos Clave Adoptados

- **Compound Engineering**: El conocimiento se acumula sesión tras sesión
- **Two-Part Loop**: Primero aprender, luego implementar
- **Self-Improving Agent**: El agente mejora leyendo sus propias instrucciones actualizadas
- **Nightly Automation**: Trabajo autónomo mientras el desarrollador duerme
...
**Autor original**: Nombre (@handle)
**Fuente**: URL o descripción
**Componentes implementados**: Lista de componentes

### Resumen

[Breve descripción de la idea principal]

### Contenido Original

> [Citar o incluir el contenido original]
...
*Este documento se actualiza cada vez que una nueva idea o artículo influye en el desarrollo del proyecto.*
